...
#include <fstream>
#include <vector>
#include <mmsystem.h> // Required for volume control
...
float volume = 1.0f; // Initial volume level
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        case 3: // Prev button
            PrevSong(hWnd);
            break;
        case 4: // Increase volume button
            IncreaseVolume();
            break;
        case 5: // Decrease volume button
            DecreaseVolume();
            break;
        }
        break;
...
void PlayCurrentSong(HWND hWnd) {
    if (!isPlaying) {
        // Set the volume level before playing the song
        waveOutSetVolume(NULL, (DWORD)(volume * 0xFFFF));
...
void DrawUI(HWND hWnd) {
    ...
    CreateWindowW(L"BUTTON", L"+", WS_VISIBLE | WS_CHILD, 380, 80, 30, 30, hWnd, (HMENU)4, NULL, NULL);
    CreateWindowW(L"BUTTON", L"-", WS_VISIBLE | WS_CHILD, 420, 80, 30, 30, hWnd, (HMENU)5, NULL, NULL);
